...
Mid-conversation intensity scaling:
    The injection engine can call scale_intensity(category, subcategory, factor)
    to shift which clips get selected at runtime without regenerating anything.
    Factor is a multiplier on the intensity metadata field:
        scale_intensity("laughs", "hearty", 0.5)  -> pulls from quieter end
        scale_intensity("laughs", "hearty", 1.5)  -> pulls from louder end
    Or use the convenience shortcuts understood by the persona core:
        "increase excitement by 25%"   -> factor = 1.25 on energy/intensity fields
        "decrease sarcasm by 50%"      -> factor = 0.50 on sarcastic subcategories
        "softer"                       -> factor = 0.70 across all categories
        "more intense"                 -> factor = 1.40 across all categories
    These do not require regenerating clips. They shift selection weights only.
"""
...
#
# INTENSITY RANGES: deliberately opened to full 0.0-1.0 spectrum.
# The old ranges clustered everything in the inner two quartiles (0.25-0.75),
# which is exactly why the library sounded flat. Real people go whisper-soft
# and genuinely loud. The full range is here so the injection engine has
# real dynamic material to draw from. The injection engine selects clips by
# matching intensity metadata to the current mood state — so a 0.95 hearty
# laugh only fires when the mood engine is actually at that energy level.
# ---------------------------------------------------------------------------
...
class CategorySpec:
    ...
    similarity_boost: float = 0.89  # locked to match reference corpus voice identity
    style: float = 0.50
...
CATEGORIES: list[CategorySpec] = [
    ...
    CategorySpec(
        ...
        emotion="amused", intensity_range=(0.05, 0.55), energy_range=(0.1, 0.65),
    ...
    CategorySpec(
        ...
        emotion="delighted", intensity_range=(0.4, 1.0), energy_range=(0.5, 1.0),
    ...
    CategorySpec(
        ...
        emotion="amused_restrained", intensity_range=(0.1, 0.65), energy_range=(0.15, 0.65),
    ...
    CategorySpec(
        ...
        emotion="anxious", intensity_range=(0.15, 0.70), energy_range=(0.2, 0.75),
    ...
    CategorySpec(
        ...
        emotion="content", intensity_range=(0.05, 0.60), energy_range=(0.1, 0.65),
    ...
    CategorySpec(
        ...
        emotion="frustrated", intensity_range=(0.2, 0.95), energy_range=(0.25, 0.95),
    ...
    CategorySpec(
        ...
        emotion="tired", intensity_range=(0.05, 0.55), energy_range=(0.05, 0.45),
    ...
    CategorySpec(
        ...
        emotion="contemplative", intensity_range=(0.05, 0.55), energy_range=(0.1, 0.60),
    ...
    CategorySpec(
        ...
        emotion="bracing", intensity_range=(0.15, 0.85), energy_range=(0.2, 0.85),
    ...
    CategorySpec(
        ...
        emotion="release", intensity_range=(0.05, 0.55), energy_range=(0.05, 0.55),
    ...
    CategorySpec(
        ...
        emotion="contemplative", intensity_range=(0.05, 0.45), energy_range=(0.1, 0.50),
    ...
    CategorySpec(
        ...
        emotion="acknowledging", intensity_range=(0.05, 0.50), energy_range=(0.1, 0.55),
    ...
    CategorySpec(
        ...
        emotion="agreeing", intensity_range=(0.05, 0.75), energy_range=(0.1, 0.75),
        tags=["agreement", "warm", "present"],
        appropriate_contexts=["agreement", "acknowledgment"],
        inappropriate_contexts=["disagreement"],
        stability=0.40, style=0.35, plain_words=True, model_id="eleven_multilingual_v2",
    ),
    CategorySpec(
        ...
        emotion="agreeing", intensity_range=(0.05, 0.70), energy_range=(0.1, 0.70),
        tags=["agreement", "validation"],
        appropriate_contexts=["validation", "acknowledgment"],
        inappropriate_contexts=["disagreement"],
        stability=0.42, style=0.32, plain_words=True, model_id="eleven_multilingual_v2",
    ),
    CategorySpec(
        category="affirmations", subcategory="mhm",
        prompts=["Mhm.", "Mm-hmm.", "Mhm, yeah.", "Mm-hmm, go on.", "Mm-hmm, okay.", "Mhm, right."],
        emotion="listening", intensity_range=(0.05, 0.55), energy_range=(0.1, 0.60),
        ...
        stability=0.48, style=0.25, plain_words=True, model_id="eleven_multilingual_v2",
    ),
    ...
    CategorySpec(
        ...
        emotion="conversational", intensity_range=(0.05, 0.50), energy_range=(0.1, 0.60),
    ...
    CategorySpec(
        ...
        emotion="conversational", intensity_range=(0.05, 0.55), energy_range=(0.1, 0.60),
    ...
    CategorySpec(
        ...
        emotion="conversational", intensity_range=(0.05, 0.45), energy_range=(0.1, 0.55),
        tags=["filler", "casual"],
...
def scale_intensity(
    ...
    """
    Runtime intensity scaling — does NOT regenerate clips.
    Multiplies intensity and energy_level metadata fields by factor for
    matching records, clamped to [0.0, 1.0]. The injection engine uses
    these fields to select which clip to play. Calling this shifts
    selection toward softer (factor < 1.0) or louder (factor > 1.0) clips.

    Examples:
        scale_intensity(records, "laughs", "hearty", 0.5)   # quieter hearty laughs
        scale_intensity(records, "reactions", None, 1.4)    # all reactions more intense
        scale_intensity(records, "sighs", "tired", 0.6)     # softer tired sighs
    """
    updated = []
    for r in metadata_records:
        if r["category"] == category and (subcategory is None or r["subcategory"] == subcategory):
            r = dict(r)
            r["intensity"] = round(min(1.0, r.get("intensity", 0.5) * factor), 3)
            r["energy_level"] = round(min(1.0, r.get("energy_level", 0.5) * factor), 3)
        updated.append(r)
    return updated
...
def isolate_paralinguistic(audio: np.ndarray, sr: int) -> np.ndarray:
    """
    Trim edge silence with gentler settings to avoid clipping soft starts/ends.
    top_db=25 and pad_ms=250 preserve breathy onsets and trailing murmurs
    that the old top_db=30/pad_ms=80 settings were cutting.
    If multiple non-silent regions exist, keep the longest one.
    """
    import librosa

    audio = trim_silence(audio, sr, top_db=25.0, pad_ms=250)
    if audio.size < int(sr * 0.1):
        return audio
    intervals = librosa.effects.split(audio, top_db=25.0, frame_length=1024, hop_length=256)
    if len(intervals) <= 1:
        return audio
    best = max(intervals, key=lambda it: it[1] - it[0])
    start, end = best
    pad = int(sr * 0.08)  # 80ms pad vs old 50ms
...
def generate_clip(
    ...
    params = GenerationParams(
        ...
        stability=jitter(spec.stability, 0.08, 0.10, 0.95),
        similarity_boost=jitter(spec.similarity_boost, 0.03, 0.80, 0.95),  # tight jitter, stays near 0.89
    ...
    if spec.plain_words:
        # plain words sit tight on silence; gentle pad only
        audio = trim_silence(audio, params.sample_rate, top_db=28.0, pad_ms=80)
